import { parseOrgSocial, parseOrgSocialTimestamp, processOrgContent, fetchOrgSocial, fetchFollowedUsers } from '../src/index.js'
...
`
  ...
  test('extractMentions - single mention', () => {
    ...
    const { extractMentions } = await import('../src/index.js')
  ...
  test('extractLinks - org-mode link format', () => {
    ...
    const { extractLinks } = await import('../src/index.js')
    const links = extractLinks(content)
    assertArrayLength(links, 1, 'Should extract one link')
    assertEquals(links[0].url, 'https://github.com/project', 'Should extract link URL')
    assertEquals(links[0].text, 'my project', 'Should extract link text')
  })
  ...
  test('extractLinks - mixed links', () => {
    ...
    const { extractLinks } = await import('../src/index.js')
    const links = extractLinks(content)
    assertArrayLength(links, 2, 'Should extract two links')
    assertEquals(links[0].url, 'https://github.com/project', 'Should extract org-mode link')
    assertEquals(links[1].url, 'https://example.com', 'Should extract plain URL')
  })
  ...
  test('extractLinks - excludes org-social mentions', () => {
    ...
    const { extractLinks } = await import('../src/index.js')
    const links = extractLinks(content)
    assertArrayLength(links, 1, 'Should extract only non-org-social links')
    assertEquals(links[0].url, 'https://example.com', 'Should extract regular link')
  })

  // Test normalizeUrl
  test('normalizeUrl - domain without protocol', () => {
    const { normalizeUrl } = await import('../src/index.js')
...
`
    ...
    assertEquals(post.replyUrl, 'https://example.com/original-post', 'Should extract reply URL')
  })
...
`
    ...
    assertTruthy(result.posts[0].content.includes('Second post'), 'Newest post should be first')
    assertTruthy(result.posts[1].content.includes('First post'), 'Oldest post should be last')
  })
...
`
  ...
  test('formatTimestampForDisplay - various time differences', () => {
    const { formatTimestampForDisplay } = await import('../src/index.js')
    const now = new Date()
  ...
  test('formatJoinDate - various dates', () => {
    const { formatJoinDate } = await import('../src/index.js')
